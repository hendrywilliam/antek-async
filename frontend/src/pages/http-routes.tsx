...
import { GetHTTPRouteYAML } from "../../wailsjs/go/main/App";
...
const HTTP_ROUTE_ACCESSORS: Accessors<kube.HTTPRouteInfo> = {
	namespace: (route) => route.namespace,
	name: (route) => route.name,
	key: (route) => `${route.namespace}/${route.name}`,
};

const HTTP_ROUTE_GROUPS: GroupOption<kube.HTTPRouteInfo>[] = [
...
const HTTP_ROUTE_COLUMNS: Column<kube.HTTPRouteInfo>[] = [
	{
		header: "Namespace",
...
const NO_HTTP_ROUTES: kube.HTTPRouteInfo[] = [];

export function HTTPRoutesPage() {
	const { state, reload } = useResource("httpRoutes");
	...
	const httpRouteRowActions = useCallback(
		(route: kube.HTTPRouteInfo) => (
			<DropdownMenu>
	...
	return (
		<>
			<ResourceTable
				accessors={HTTP_ROUTE_ACCESSORS}
				columns={HTTP_ROUTE_COLUMNS}
				error={routes?.error ?? ""}
				groups={HTTP_ROUTE_GROUPS}
				loaded={routes?.loaded ?? false}
				loading={routes?.loading ?? false}
				noun="HTTPRoute"
				onRetry={reload}
				rowActions={httpRouteRowActions}
				rows={routes?.items ?? NO_HTTP_ROUTES}
			/>
			<YamlDrawer
				fetchYaml={GetHTTPRouteYAML}
				noun="HTTPRoute"
